...
public class SelectEyeColorView : InOutUView
{
    [Header("列表组件")]
    public List<Color_item> list;
    ...
    {
        base.DoCreat();
        CreatePlayerViewMgr.SelectEyeColorView = this;
    ...
    void initListData()
    {
        ...
        DressVo vo;
        for (int i = 13001; i < 13008; i++)
        {
            vo = new DressVo();
            vo.cfg = ExcelMgr.TDCharacter.Get(i + "");
            // vo.cl = StringTool.ToColor(kvp.Value.color);
            vo.type = 1;
            dataList.Add(vo);
    ...
    void KeyDown(KeyCode keyCode)
    {
        switch (keyCode)
        {
            ...
            case KeyCode.J:
                ...
                {
                    DoClose();
                    EventCenter.ins.EventTrigger("create_player_step6");
                });
                break;
            case KeyCode.K:
                EventCenter.ins.RemoveEventListener<KeyCode>("KeyDown",KeyDown);
